...
int* dynamicArray(int n, int queries_rows, int queries_columns, int** queries, int* result_count) {
    ...
    int* sizes = calloc(n, sizeof(int));        // sizes of each sequence
    int* capacities = calloc(n, sizeof(int));   // capacities of each sequence
    int* result = malloc(queries_rows * sizeof(int));
    int resIndex = 0;

    for (int i = 0; i < n; i++) {
        capacities[i] = 1;  // initial capacity
        seqList[i] = malloc(capacities[i] * sizeof(int));
    }
